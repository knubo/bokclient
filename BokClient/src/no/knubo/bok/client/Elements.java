package no.knubo.bok.client;

/**
 * Interface to represent the constants contained in resource bundle:
 * 	'/Users/knuterikborgen/kode/workspace/BokClient/src/no/knubo/bok/client/Elements.properties'.
 */
public interface Elements extends com.google.gwt.i18n.client.ConstantsWithLookup {
  
  /**
   * Translated "Elements".
   * 
   * @return translated "Elements"
   */
  @DefaultStringValue("Elements")
  @Key("ClassName")
  String ClassName();

  /**
   * Translated "Avbryt".
   * 
   * @return translated "Avbryt"
   */
  @DefaultStringValue("Avbryt")
  @Key("abort")
  String abort();

  /**
   * Translated "Tilgang".
   * 
   * @return translated "Tilgang"
   */
  @DefaultStringValue("Tilgang")
  @Key("access")
  String access();

  /**
   * Translated "Aksjon".
   * 
   * @return translated "Aksjon"
   */
  @DefaultStringValue("Aksjon")
  @Key("action")
  String action();

  /**
   * Translated "Ny/Endre kategori".
   * 
   * @return translated "Ny/Endre kategori"
   */
  @DefaultStringValue("Ny/Endre kategori")
  @Key("alter_categories")
  String alter_categories();

  /**
   * Translated "Ny/Endre person".
   * 
   * @return translated "Ny/Endre person"
   */
  @DefaultStringValue("Ny/Endre person")
  @Key("alter_person")
  String alter_person();

  /**
   * Translated "Ny/Endre plassering".
   * 
   * @return translated "Ny/Endre plassering"
   */
  @DefaultStringValue("Ny/Endre plassering")
  @Key("alter_placements")
  String alter_placements();

  /**
   * Translated "Ny/Endre forlegger".
   * 
   * @return translated "Ny/Endre forlegger"
   */
  @DefaultStringValue("Ny/Endre forlegger")
  @Key("alter_publishers")
  String alter_publishers();

  /**
   * Translated "Ny/Endre serie ".
   * 
   * @return translated "Ny/Endre serie "
   */
  @DefaultStringValue("Ny/Endre serie ")
  @Key("alter_series")
  String alter_series();

  /**
   * Translated "Tilbake".
   * 
   * @return translated "Tilbake"
   */
  @DefaultStringValue("Tilbake")
  @Key("back")
  String back();

  /**
   * Translated "Last ned forrige sikkerhetskopi".
   * 
   * @return translated "Last ned forrige sikkerhetskopi"
   */
  @DefaultStringValue("Last ned forrige sikkerhetskopi")
  @Key("backup_download_last")
  String backup_download_last();

  /**
   * Translated "Laster ned zipfil".
   * 
   * @return translated "Laster ned zipfil"
   */
  @DefaultStringValue("Laster ned zipfil")
  @Key("backup_downloading")
  String backup_downloading();

  /**
   * Translated "Klarte ikke operasjonen:".
   * 
   * @return translated "Klarte ikke operasjonen:"
   */
  @DefaultStringValue("Klarte ikke operasjonen:")
  @Key("backup_error")
  String backup_error();

  /**
   * Translated "Starter sikkerhetskopiering".
   * 
   * @return translated "Starter sikkerhetskopiering"
   */
  @DefaultStringValue("Starter sikkerhetskopiering")
  @Key("backup_init")
  String backup_init();

  /**
   * Translated "Siste sikkerhetskopi ".
   * 
   * @return translated "Siste sikkerhetskopi "
   */
  @DefaultStringValue("Siste sikkerhetskopi ")
  @Key("backup_last")
  String backup_last();

  /**
   * Translated "Progresjon".
   * 
   * @return translated "Progresjon"
   */
  @DefaultStringValue("Progresjon")
  @Key("backup_progress")
  String backup_progress();

  /**
   * Translated "Start sikkerhetskopiering".
   * 
   * @return translated "Start sikkerhetskopiering"
   */
  @DefaultStringValue("Start sikkerhetskopiering")
  @Key("backup_start")
  String backup_start();

  /**
   * Translated "Tabell:".
   * 
   * @return translated "Tabell:"
   */
  @DefaultStringValue("Tabell:")
  @Key("backup_table")
  String backup_table();

  /**
   * Translated "Antall tabeller som skal sikkerhetskopieres:".
   * 
   * @return translated "Antall tabeller som skal sikkerhetskopieres:"
   */
  @DefaultStringValue("Antall tabeller som skal sikkerhetskopieres:")
  @Key("backup_table_count")
  String backup_table_count();

  /**
   * Translated "Pakker filer".
   * 
   * @return translated "Pakker filer"
   */
  @DefaultStringValue("Pakker filer")
  @Key("backup_zipping")
  String backup_zipping();

  /**
   * Translated "Forfatter".
   * 
   * @return translated "Forfatter"
   */
  @DefaultStringValue("Forfatter")
  @Key("book_author")
  String book_author();

  /**
   * Translated "Medforfatter".
   * 
   * @return translated "Medforfatter"
   */
  @DefaultStringValue("Medforfatter")
  @Key("book_coauthor")
  String book_coauthor();

  /**
   * Translated "Antall bøker:".
   * 
   * @return translated "Antall bøker:"
   */
  @DefaultStringValue("Antall bøker:")
  @Key("book_count")
  String book_count();

  /**
   * Translated "Utgave".
   * 
   * @return translated "Utgave"
   */
  @DefaultStringValue("Utgave")
  @Key("book_edition")
  String book_edition();

  /**
   * Translated "Redaktør".
   * 
   * @return translated "Redaktør"
   */
  @DefaultStringValue("Redaktør")
  @Key("book_editor")
  String book_editor();

  /**
   * Translated "Illustratør".
   * 
   * @return translated "Illustratør"
   */
  @DefaultStringValue("Illustratør")
  @Key("book_illustrator")
  String book_illustrator();

  /**
   * Translated "Opplag".
   * 
   * @return translated "Opplag"
   */
  @DefaultStringValue("Opplag")
  @Key("book_impression")
  String book_impression();

  /**
   * Translated "ISBN".
   * 
   * @return translated "ISBN"
   */
  @DefaultStringValue("ISBN")
  @Key("book_isbn")
  String book_isbn();

  /**
   * Translated "Neste boknummer".
   * 
   * @return translated "Neste boknummer"
   */
  @DefaultStringValue("Neste boknummer")
  @Key("book_next_user_number")
  String book_next_user_number();

  /**
   * Translated "Boknummer".
   * 
   * @return translated "Boknummer"
   */
  @DefaultStringValue("Boknummer")
  @Key("book_number")
  String book_number();

  /**
   * Translated "Org. tittel".
   * 
   * @return translated "Org. tittel"
   */
  @DefaultStringValue("Org. tittel")
  @Key("book_org_title")
  String book_org_title();

  /**
   * Translated "Personer:".
   * 
   * @return translated "Personer:"
   */
  @DefaultStringValue("Personer:")
  @Key("book_person_info")
  String book_person_info();

  /**
   * Translated "Plassering".
   * 
   * @return translated "Plassering"
   */
  @DefaultStringValue("Plassering")
  @Key("book_placement")
  String book_placement();

  /**
   * Translated "Pris".
   * 
   * @return translated "Pris"
   */
  @DefaultStringValue("Pris")
  @Key("book_price")
  String book_price();

  /**
   * Translated "Forlegger".
   * 
   * @return translated "Forlegger"
   */
  @DefaultStringValue("Forlegger")
  @Key("book_publisher")
  String book_publisher();

  /**
   * Translated "Oppleser".
   * 
   * @return translated "Oppleser"
   */
  @DefaultStringValue("Oppleser")
  @Key("book_read_by")
  String book_read_by();

  /**
   * Translated "Registrer bok".
   * 
   * @return translated "Registrer bok"
   */
  @DefaultStringValue("Registrer bok")
  @Key("book_register_book")
  String book_register_book();

  /**
   * Translated "Nr i serie".
   * 
   * @return translated "Nr i serie"
   */
  @DefaultStringValue("Nr i serie")
  @Key("book_serie_nmb")
  String book_serie_nmb();

  /**
   * Translated "Serie".
   * 
   * @return translated "Serie"
   */
  @DefaultStringValue("Serie")
  @Key("book_series")
  String book_series();

  /**
   * Translated "Undertittel".
   * 
   * @return translated "Undertittel"
   */
  @DefaultStringValue("Undertittel")
  @Key("book_subtitle")
  String book_subtitle();

  /**
   * Translated "Tittel".
   * 
   * @return translated "Tittel"
   */
  @DefaultStringValue("Tittel")
  @Key("book_title")
  String book_title();

  /**
   * Translated "Oversetter".
   * 
   * @return translated "Oversetter"
   */
  @DefaultStringValue("Oversetter")
  @Key("book_translator")
  String book_translator();

  /**
   * Translated "Utgitt år".
   * 
   * @return translated "Utgitt år"
   */
  @DefaultStringValue("Utgitt år")
  @Key("book_year_published")
  String book_year_published();

  /**
   * Translated "Skrevet år".
   * 
   * @return translated "Skrevet år"
   */
  @DefaultStringValue("Skrevet år")
  @Key("book_year_written")
  String book_year_written();

  /**
   * Translated "Avbryt".
   * 
   * @return translated "Avbryt"
   */
  @DefaultStringValue("Avbryt")
  @Key("cancel")
  String cancel();

  /**
   * Translated "Kategori".
   * 
   * @return translated "Kategori"
   */
  @DefaultStringValue("Kategori")
  @Key("categories")
  String categories();

  /**
   * Translated "Kategori".
   * 
   * @return translated "Kategori"
   */
  @DefaultStringValue("Kategori")
  @Key("category")
  String category();

  /**
   * Translated "Nullstill".
   * 
   * @return translated "Nullstill"
   */
  @DefaultStringValue("Nullstill")
  @Key("clear")
  String clear();

  /**
   * Translated "Lukk".
   * 
   * @return translated "Lukk"
   */
  @DefaultStringValue("Lukk")
  @Key("close")
  String close();

  /**
   * Translated "Slett".
   * 
   * @return translated "Slett"
   */
  @DefaultStringValue("Slett")
  @Key("delete")
  String delete();

  /**
   * Translated "Bibsys".
   * 
   * @return translated "Bibsys"
   */
  @DefaultStringValue("Bibsys")
  @Key("external_bibsys")
  String external_bibsys();

  /**
   * Translated "Bokkilden".
   * 
   * @return translated "Bokkilden"
   */
  @DefaultStringValue("Bokkilden")
  @Key("external_bokkilden")
  String external_bokkilden();

  /**
   * Translated "Eksternt oppslag mot ISBN nummer".
   * 
   * @return translated "Eksternt oppslag mot ISBN nummer"
   */
  @DefaultStringValue("Eksternt oppslag mot ISBN nummer")
  @Key("external_isbn")
  String external_isbn();

  /**
   * Translated "Se kildeside".
   * 
   * @return translated "Se kildeside"
   */
  @DefaultStringValue("Se kildeside")
  @Key("external_link")
  String external_link();

  /**
   * Translated "Noter".
   * 
   * @return translated "Noter"
   */
  @DefaultStringValue("Noter")
  @Key("external_note")
  String external_note();

  /**
   * Translated "Full tilgang".
   * 
   * @return translated "Full tilgang"
   */
  @DefaultStringValue("Full tilgang")
  @Key("full_access")
  String full_access();

  /**
   * Translated "Id".
   * 
   * @return translated "Id"
   */
  @DefaultStringValue("Id")
  @Key("id")
  String id();

  /**
   * Translated "Info".
   * 
   * @return translated "Info"
   */
  @DefaultStringValue("Info")
  @Key("info")
  String info();

  /**
   * Translated "Logg".
   * 
   * @return translated "Logg"
   */
  @DefaultStringValue("Logg")
  @Key("log")
  String log();

  /**
   * Translated "Logg inn".
   * 
   * @return translated "Logg inn"
   */
  @DefaultStringValue("Logg inn")
  @Key("login")
  String login();

  /**
   * Translated "Logg ut".
   * 
   * @return translated "Logg ut"
   */
  @DefaultStringValue("Logg ut")
  @Key("logout")
  String logout();

  /**
   * Translated "Bøker".
   * 
   * @return translated "Bøker"
   */
  @DefaultStringValue("Bøker")
  @Key("menu_books")
  String menu_books();

  /**
   * Translated "Info".
   * 
   * @return translated "Info"
   */
  @DefaultStringValue("Info")
  @Key("menu_info")
  String menu_info();

  /**
   * Translated "Logg ut".
   * 
   * @return translated "Logg ut"
   */
  @DefaultStringValue("Logg ut")
  @Key("menu_logout")
  String menu_logout();

  /**
   * Translated "Rapporter".
   * 
   * @return translated "Rapporter"
   */
  @DefaultStringValue("Rapporter")
  @Key("menu_reports")
  String menu_reports();

  /**
   * Translated "Innstillinger".
   * 
   * @return translated "Innstillinger"
   */
  @DefaultStringValue("Innstillinger")
  @Key("menu_settings")
  String menu_settings();

  /**
   * Translated "Om bokbasen".
   * 
   * @return translated "Om bokbasen"
   */
  @DefaultStringValue("Om bokbasen")
  @Key("menuitem_about")
  String menuitem_about();

  /**
   * Translated "Sikkerhetskopi".
   * 
   * @return translated "Sikkerhetskopi"
   */
  @DefaultStringValue("Sikkerhetskopi")
  @Key("menuitem_backup")
  String menuitem_backup();

  /**
   * Translated "Søk etter kategorier".
   * 
   * @return translated "Søk etter kategorier"
   */
  @DefaultStringValue("Søk etter kategorier")
  @Key("menuitem_book_categories")
  String menuitem_book_categories();

  /**
   * Translated "Hurtigoppslag på bok".
   * 
   * @return translated "Hurtigoppslag på bok"
   */
  @DefaultStringValue("Hurtigoppslag på bok")
  @Key("menuitem_book_fast_search")
  String menuitem_book_fast_search();

  /**
   * Translated "Søk etter personer".
   * 
   * @return translated "Søk etter personer"
   */
  @DefaultStringValue("Søk etter personer")
  @Key("menuitem_book_people")
  String menuitem_book_people();

  /**
   * Translated "Søk etter plasseringer".
   * 
   * @return translated "Søk etter plasseringer"
   */
  @DefaultStringValue("Søk etter plasseringer")
  @Key("menuitem_book_placements")
  String menuitem_book_placements();

  /**
   * Translated "Søk etter forleggere".
   * 
   * @return translated "Søk etter forleggere"
   */
  @DefaultStringValue("Søk etter forleggere")
  @Key("menuitem_book_publishers")
  String menuitem_book_publishers();

  /**
   * Translated "Søk etter bok".
   * 
   * @return translated "Søk etter bok"
   */
  @DefaultStringValue("Søk etter bok")
  @Key("menuitem_book_search")
  String menuitem_book_search();

  /**
   * Translated "Søk etter serier".
   * 
   * @return translated "Søk etter serier"
   */
  @DefaultStringValue("Søk etter serier")
  @Key("menuitem_book_series")
  String menuitem_book_series();

  /**
   * Translated "Vis logger".
   * 
   * @return translated "Vis logger"
   */
  @DefaultStringValue("Vis logger")
  @Key("menuitem_log")
  String menuitem_log();

  /**
   * Translated "Logg ut".
   * 
   * @return translated "Logg ut"
   */
  @DefaultStringValue("Logg ut")
  @Key("menuitem_logout")
  String menuitem_logout();

  /**
   * Translated "Registrer bok".
   * 
   * @return translated "Registrer bok"
   */
  @DefaultStringValue("Registrer bok")
  @Key("menuitem_new_book")
  String menuitem_new_book();

  /**
   * Translated "Antall bøker per kategori".
   * 
   * @return translated "Antall bøker per kategori"
   */
  @DefaultStringValue("Antall bøker per kategori")
  @Key("menuitem_report_book_category")
  String menuitem_report_book_category();

  /**
   * Translated "Siste registrerte bøker".
   * 
   * @return translated "Siste registrerte bøker"
   */
  @DefaultStringValue("Siste registrerte bøker")
  @Key("menuitem_report_book_last_registered")
  String menuitem_report_book_last_registered();

  /**
   * Translated "Antall bøker per serie".
   * 
   * @return translated "Antall bøker per serie"
   */
  @DefaultStringValue("Antall bøker per serie")
  @Key("menuitem_report_book_series")
  String menuitem_report_book_series();

  /**
   * Translated "Bøker per person".
   * 
   * @return translated "Bøker per person"
   */
  @DefaultStringValue("Bøker per person")
  @Key("menuitem_report_books_author")
  String menuitem_report_books_author();

  /**
   * Translated "Bøker uten plassering".
   * 
   * @return translated "Bøker uten plassering"
   */
  @DefaultStringValue("Bøker uten plassering")
  @Key("menuitem_report_no_placement")
  String menuitem_report_no_placement();

  /**
   * Translated "Antall bøker per plassering ".
   * 
   * @return translated "Antall bøker per plassering "
   */
  @DefaultStringValue("Antall bøker per plassering ")
  @Key("menuitem_report_placement")
  String menuitem_report_placement();

  /**
   * Translated "Top 30 forfattere".
   * 
   * @return translated "Top 30 forfattere"
   */
  @DefaultStringValue("Top 30 forfattere")
  @Key("menuitem_report_top_author")
  String menuitem_report_top_author();

  /**
   * Translated "Brukere".
   * 
   * @return translated "Brukere"
   */
  @DefaultStringValue("Brukere")
  @Key("menuitem_useradm")
  String menuitem_useradm();

  /**
   * Translated "Standardverdier".
   * 
   * @return translated "Standardverdier"
   */
  @DefaultStringValue("Standardverdier")
  @Key("menuitem_values")
  String menuitem_values();

  /**
   * Translated "Melding".
   * 
   * @return translated "Melding"
   */
  @DefaultStringValue("Melding")
  @Key("message")
  String message();

  /**
   * Translated "Navn".
   * 
   * @return translated "Navn"
   */
  @DefaultStringValue("Navn")
  @Key("name")
  String name();

  /**
   * Translated "Tidspunkt".
   * 
   * @return translated "Tidspunkt"
   */
  @DefaultStringValue("Tidspunkt")
  @Key("occured")
  String occured();

  /**
   * Translated "Passord".
   * 
   * @return translated "Passord"
   */
  @DefaultStringValue("Passord")
  @Key("password")
  String password();

  /**
   * Translated "Fornavn".
   * 
   * @return translated "Fornavn"
   */
  @DefaultStringValue("Fornavn")
  @Key("person_firstname")
  String person_firstname();

  /**
   * Translated "Etternavn".
   * 
   * @return translated "Etternavn"
   */
  @DefaultStringValue("Etternavn")
  @Key("person_lastname")
  String person_lastname();

  /**
   * Translated "Plassering".
   * 
   * @return translated "Plassering"
   */
  @DefaultStringValue("Plassering")
  @Key("placements")
  String placements();

  /**
   * Translated "Forlegger".
   * 
   * @return translated "Forlegger"
   */
  @DefaultStringValue("Forlegger")
  @Key("publishers")
  String publishers();

  /**
   * Translated "Kun lesetilgang".
   * 
   * @return translated "Kun lesetilgang"
   */
  @DefaultStringValue("Kun lesetilgang")
  @Key("read_only_access")
  String read_only_access();

  /**
   * Translated "Redusert skrivetilgang".
   * 
   * @return translated "Redusert skrivetilgang"
   */
  @DefaultStringValue("Redusert skrivetilgang")
  @Key("reduced_write_access")
  String reduced_write_access();

  /**
   * Translated "Lagre".
   * 
   * @return translated "Lagre"
   */
  @DefaultStringValue("Lagre")
  @Key("save")
  String save();

  /**
   * Translated "Søk".
   * 
   * @return translated "Søk"
   */
  @DefaultStringValue("Søk")
  @Key("search")
  String search();

  /**
   * Translated "Serie".
   * 
   * @return translated "Serie"
   */
  @DefaultStringValue("Serie")
  @Key("series")
  String series();

  /**
   * Translated "Om bokbasen".
   * 
   * @return translated "Om bokbasen"
   */
  @DefaultStringValue("Om bokbasen")
  @Key("title_about")
  String title_about();

  /**
   * Translated "Sikkerhetskopi av database".
   * 
   * @return translated "Sikkerhetskopi av database"
   */
  @DefaultStringValue("Sikkerhetskopi av database")
  @Key("title_back")
  String title_back();

  /**
   * Translated "Bok".
   * 
   * @return translated "Bok"
   */
  @DefaultStringValue("Bok")
  @Key("title_book_quickview")
  String title_book_quickview();

  /**
   * Translated "Endre bok".
   * 
   * @return translated "Endre bok"
   */
  @DefaultStringValue("Endre bok")
  @Key("title_change_book")
  String title_change_book();

  /**
   * Translated "Ny/endre bruker".
   * 
   * @return translated "Ny/endre bruker"
   */
  @DefaultStringValue("Ny/endre bruker")
  @Key("title_edit_new_user")
  String title_edit_new_user();

  /**
   * Translated "Registrer ny bok".
   * 
   * @return translated "Registrer ny bok"
   */
  @DefaultStringValue("Registrer ny bok")
  @Key("title_new_book")
  String title_new_book();

  /**
   * Translated "Søk etter bok".
   * 
   * @return translated "Søk etter bok"
   */
  @DefaultStringValue("Søk etter bok")
  @Key("title_search")
  String title_search();

  /**
   * Translated "Brukere".
   * 
   * @return translated "Brukere"
   */
  @DefaultStringValue("Brukere")
  @Key("title_user_adm")
  String title_user_adm();

  /**
   * Translated "Bruk valgte felter".
   * 
   * @return translated "Bruk valgte felter"
   */
  @DefaultStringValue("Bruk valgte felter")
  @Key("use")
  String use();

  /**
   * Translated "Brukernavn".
   * 
   * @return translated "Brukernavn"
   */
  @DefaultStringValue("Brukernavn")
  @Key("user")
  String user();

  /**
   * Translated "Ny bruker".
   * 
   * @return translated "Ny bruker"
   */
  @DefaultStringValue("Ny bruker")
  @Key("userEditView_newButton")
  String userEditView_newButton();
}
